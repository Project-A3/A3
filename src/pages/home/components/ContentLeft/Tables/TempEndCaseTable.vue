<template>
  <div class="block table-block">
    <div class="block-title-outside scroll-fix is-flex is-justify-content-space-between is-align-items-center">
      <p>
        <cathay-translate code="Component_TempEndCaseTable_001" /> <span>({{ tempEndCaseData.length }})</span>
      </p>
        ...
        <StickyThead>
          <tr class="scroll-fix">
            <SortingThead sortKey="APLY_DATE" :currentSortKey="sortName" :isSortDesc="isSortDesc" :onClickSort="setSort">
              <cathay-translate code="Component_TempEndCaseTable_002" />
            </SortingThead>
            <SortingThead sortKey="APLY_NO" :currentSortKey="sortName" :isSortDesc="isSortDesc" :onClickSort="setSort">
              <cathay-translate code="Component_TempEndCaseTable_003" />
            </SortingThead>
            <SortingThead sortKey="CLAM_CAT" :currentSortKey="sortName" :isSortDesc="isSortDesc" :onClickSort="setSort">
              <cathay-translate code="Component_TempEndCaseTable_004" />
            </SortingThead>
            <th><cathay-translate code="Component_TempEndCaseTable_005" /></th>
            <SortingThead sortKey="DECD_EMP_NAME" :currentSortKey="sortName" :isSortDesc="isSortDesc" :onClickSort="setSort">
              <cathay-translate code="Component_TempEndCaseTable_006" />
            </SortingThead>
            <SortingThead sortKey="END_CASE_DATE" :currentSortKey="sortName" :isSortDesc="isSortDesc" :onClickSort="setSort">
              <cathay-translate code="Component_TempEndCaseTable_007" />
            </SortingThead>
            <th><cathay-translate code="Component_TempEndCaseTable_008" /></th>
            <th><cathay-translate code="Component_TempEndCaseTable_009" /></th>
            <th><cathay-translate code="Component_TempEndCaseTable_010" /></th>
          </tr>
        </StickyThead>
        <tbody>
          <tr v-for="endCase in tempEndCaseData" :key="endCase.APLY_NO">
            ...
            <td width="128">
              ...
              >
                <cathay-translate code="Component_TempEndCaseTable_011" />
              </button>
            </td>
            <td width="176">
...
<script setup>
  ...
  const tempEndCaseData = await useCaseTable('tempEndCaseList_mapList', defaultDate.start, defaultDate.end);
  ...
  onMounted(() => {
    ...
    picker.on('select', async (date) => {
      ...
      if (Math.abs(diffDay) >= 30) {
        $swal.fail(language.translate('Component_TempEndCaseTable_012'));
      } else {
        ...
        const response = await useCaseTable('tempEndCaseList_mapList', startDate, endDate);
        dataCnt.value.decdCnt_map.CLAM_TEMP_END_CASE_CNT = response.length;
        tempEndCaseData.length = 0;
        tempEndCaseData.push(...response);
      }
    });
  ...
  const { sortName, isSortDesc, setSort } = useTableSort(tempEndCaseData);
