    ...
    <StickyThead>
      <tr class="scroll-fix">
        <SortingThead sortKey="APLY_NO" :currentSortKey="sortName" :isSortDesc="isSortDesc" :onClickSort="setSort">
          <cathay-translate code="Component_ApprInvsTable_001" />
        </SortingThead>
        <th><cathay-translate code="Component_ApprInvsTable_002" /></th>
        <th><cathay-translate code="Component_ApprInvsTable_003" /></th>
        <SortingThead sortKey="OCR_NAME" :currentSortKey="sortName" :isSortDesc="isSortDesc" :onClickSort="setSort">
          <cathay-translate code="Component_ApprInvsTable_004" />
        </SortingThead>
        <SortingThead sortKey="CASE_DATE" :currentSortKey="sortName" :isSortDesc="isSortDesc" :onClickSort="setSort">
          <cathay-translate code="Component_ApprInvsTable_005" />
        </SortingThead>
      </tr>
    </StickyThead>
    <tbody>
      <tr v-for="decd in decdData" :key="decd.APLY_NO">
        <td style="width: 80px">
          <a href="#" @click.stop.prevent="onClickApplyNo(decd.APLY_NO)">
            {{ decd.APLY_NO }}
          </a>
          <br />
...
<script setup>
  ...
  const decdData = reactive(props.aprvInvs_mapList);

  const { getVNDate } = useMoment();

  const { sortName, isSortDesc, setSort } = useTableSort(decdData);
  const onClickApplyNo = (applyNo) => {
    window.open(`/servlet/HttpDispatcher/AHB0_3001/prompt?PROCESS_NUM=${applyNo}`);
  };
</script>
